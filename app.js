...
function addLayer(name) {
  ...
  const layer = {
    ...
    visible: true, name: name || `Layer ${layers.length+1}`,
    undoStack: [], redoStack: []
  };
  layers.push(layer);
  refreshZIndex();
...
function initColorPicker() {
  ...
  drawSVSquare();
  if (svMobCtx) {
    drawHueBar(hueMobCtx, hueMob.width, hueMob.height);
    drawSVSquareMob();
  }
  ...
  if (svMob) {
    svMob.addEventListener('mousedown',   e => { dSVM  = true; pickSV(e,  svMob);  });
    hueMob.addEventListener('mousedown',  e => { dHueM = true; pickHue(e, hueMob); });
    svMob.addEventListener('touchstart',  e => { dSVM  = true; pickSV(e.touches[0],  svMob);  }, { passive: true });
    hueMob.addEventListener('touchstart', e => { dHueM = true; pickHue(e.touches[0], hueMob); }, { passive: true });
    window.addEventListener('touchmove',  e => {
      if (dSVM)  pickSV(e.touches[0],  svMob);
      if (dHueM) pickHue(e.touches[0], hueMob);
    }, { passive: true });
    window.addEventListener('touchend', () => { dSVM = dHueM = false; });
  }

  window.addEventListener('mousemove', e => {
    if (dSV)  pickSV(e,  svCanvas);
    if (dHue) pickHue(e, hueCanvas);
    if (dSVM)  pickSV(e,  svMob);
    if (dHueM) pickHue(e, hueMob);
  });
...
function _drawSV(ctx, w, h) {
  ...
  ctx.strokeStyle = pickerV > 0.5 ? '#000' : '#fff';
...
function onMouseDown(e) {
  ...
  if (imgState) {
    ...
    if (handle) {
      ...
      imgState.dragOffX = x - imgState.x; imgState.dragOffY = y - imgState.y;
    } else {
      stampImage();
    }
    return;
...
function beginTextInput(x, y) {
  ...
  textInputEl.style.left       = (x * scaleX) + 'px';
...
function paintLine(x, y) {
  ...
  if (currentTool === 'eraser') {
    ...
    ctx.beginPath(); ctx.moveTo(lastX, lastY); ctx.lineTo(x, y);
    ctx.strokeStyle = colorStr(); ctx.lineWidth = brushSize;
...
function renderShape(ctx, x0, y0, x1, y1) {
  ctx.save();
  ctx.strokeStyle = colorStr(); ctx.fillStyle = colorStr();
...
function sampleColor(x, y) {
  ...
  tCtx.fillStyle='#fff'; tCtx.fillRect(0,0,CANVAS_W,CANVAS_H);
...
function bindUIEvents() {
  ...
  textInputEl.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (textActive) { textActive = false; _commitText(); }
    }
    if (e.key === 'Escape') {
      e.preventDefault();
      textActive = false;
  ...
  document.getElementById('refresh-btn').addEventListener('click', () => {
    ...
    if (textActive) { textActive = false; textInputEl.style.display = 'none'; textInputEl.value = ''; }
